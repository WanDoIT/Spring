...
<%@ taglib uri="http://java.sun.com/jsp/jstl/fmt" prefix="fmt" %>
	...
	<div id="container">
		...
		<div class="bodytext_area box_inner">
			<form action="notice_list" class="minisrch_form" name="actionForm"> 
				...
				<fieldset>
					...
					<input type="submit" class="btn_srch" value="검색" style="height: 35px;">
		...
		<%-- 	컨트롤러에서 사용자 클래스를 매개변수로 지정을 하면 이름이 클래스명소문자로 지정돼서 자동으로 다음화면으로 전달됩니다.
			${criteria } --%>
			<div class="pagination">
				<!-- 5.맨처음 페이지네이션 -->
				...
				<!-- 1.페이지네이션 -->
				<c:forEach var="num" begin="${pageVO.start }" end="${pageVO.end }" >
				<a href="notice_list?page=${num }&amount=${pageVO.amount }&searchType=${criteria.searchType}&searchName=${criteria.searchName}"><span class="pagenum ${pageVO.page == num ? 'currentpage' :''}">${num }</span></a>
				</c:forEach>
				<!-- 2.다음페이지네이션 -->
				<c:if test="${pageVO.next }">
